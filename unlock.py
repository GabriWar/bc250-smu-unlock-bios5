#!/usr/bin/env python3
"""
BC-250 SMU Unlock for BIOS 5 (SMU firmware 0.58.7.1)

Ported from rw-r-r-0644/bc250-smu-unlock (BIOS 3).
...
All other logic identical to BIOS 3 version.
"""
import ctypes
import os
import struct
...
# q2 msg 0x23 — UNCHANGED from BIOS 3
...
# q2 msg 0xA — UNCHANGED from BIOS 3
...
# BIOS 5 CHANGES: debug gate moved +8 bytes
DBG_DISABLE = 0x7B44        # was 0x7B3C in BIOS 3
NEW_ENTRY_ADDR = 0x7B40     # was 0x7B38 in BIOS 3 (DBG_DISABLE - 4)

PROBE_ADDR = 0x0005A870     # SMN address, not firmware offset
...
def _do_unlock(smu: Bc250Smu, va, phys) -> bool:
    ...
    # search for the first block of 72 zero-bytes below DBG_DISABLE (in practice ~7 chunks)
    buf = b""
    P = None
    for a in range(DBG_DISABLE - 72, 0x3054 - 72, -72):
        buf = smu.smu_read(a, 18) + buf
        for c in range(DBG_DISABLE - 0x20, max(0x3070, a + 0x1C) - 1, -4):
        ...
        if P is not None:
            break
    if P is None:
        raise SmuError("no dead-zone base below DBG_DISABLE - aborting")
    N = (DBG_DISABLE - 0x1C - P) // 4
    ...
    smu.smu_memset32(0x7950, 0, 0x4C // 4) # staged zone (unchanged)
    smu.smu_memset32(0x18DF0, 0, 0x1F0 // 4) # ring slots + counters (unchanged)
    fixup = [
        (NEW_ENTRY_ADDR, 0),    # was 0x7B38 in BIOS 3, now 0x7B40
        (0x19780, 0x3F794),     # unchanged
        (0x19784, 0x3E000),     # unchanged
        (0x19788, 0),           # unchanged
        (0x1978C, 0),           # unchanged
        (0x17E1C, 0x8B08)       # unchanged
    ]
    for a, v in fixup:
        smu.smu_write32(a, v)

    print("fixup done; only 0x%X (dbg) should differ from boot" % DBG_DISABLE)
